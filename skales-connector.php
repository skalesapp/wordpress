...
function skales_detect_plugins() {
    ...
    $plugins[] = [
        'name' => 'WP Rocket',
        'slug' => 'wp-rocket',
        'active' => is_plugin_active('wp-rocket/wp-rocket.php') || function_exists('rocket_clean_domain'),
        'capabilities' => ['cache_clear'],
    ];
...
function skales_route_upload_media($request) {
    ...
    if (!empty($params['base64'])) {
        ...
        // SECURITY: Extension blocklist — these extensions are never allowed,
        // regardless of what MIME type the decoded bytes claim to be.
        $dangerous_extensions = ['php', 'phtml', 'phar', 'php3', 'php4', 'php5', 'php7',
                                 'pl', 'py', 'cgi', 'sh', 'exe', 'bat', 'cmd', 'com',
                                 'vbs', 'ps1', 'js', 'jsp', 'asp', 'aspx', 'htaccess'];
        ...
        // SECURITY: MIME allowlist — only permit common media types.
        $allowed_mimes = [
            // images
        ...
        // Write decoded bytes to a temp file so wp_check_filetype_and_ext
        // can sniff the real MIME (WordPress uses finfo under the hood).
        $tmp_path = wp_tempnam($filename);
        if (!$tmp_path) {
            return new WP_Error('upload_tempfile_failed',
                'Could not create temp file for validation',
                ['status' => 500]);
        }
        ...
        if (empty($check['type']) || !in_array($check['type'], $allowed_mimes, true)) {
            return new WP_Error('invalid_file_type',
                'File MIME type not allowed: ' . ($check['type'] ?? 'unknown'),
                ['status' => 400]);
        }
